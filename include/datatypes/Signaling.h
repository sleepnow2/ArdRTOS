...
public:
     ...
     * 
     */
    Mutex() : _lock(true) , _locking_task(0xFF) {};
    ...
    void lock() {
        ...
        while (!_lock) {
            ...
            // data from changing while we are working on it
            noInterrupts();
            // if someone is holding it, and we have not already given them priority
            if (oldLockingTask != _locking_task) {
                OS.donatePriority(_locking_task);
                oldLockingTask = _locking_task;
            }
        }
        // update lock
        _lock = false;
